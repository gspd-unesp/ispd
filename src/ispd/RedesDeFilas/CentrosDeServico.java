...
{
    private int idCS;
	private int tipo; // 0: maquina; 1: cluster; 2: conexao de rede simples; 3: Internet
    private int numMaxServidores;  //Capacidade maxima de servidores que podem ser adicionados ao CS. 
	                               //Esse numero considera apenas servidores de processamento (mestre e escravos)
								   //ou seja, se esse numero for 100, o servidor tera 101 servidores, sendo:
	                               //1 servidor de comunicacao, 1 mestre e 99 escravosm (os dois ultimos sao do tipo "processamento!")
    private int numAtualServidores;  //Quantidade de servidores ja adicionados ao CS OBS: Em qualquer momento numAtualServidores e no maximo igual a numMaxServidores
	...
	public CentrosDeServico( int identCS, int tp, int nMaxServ, int nMaxFilas, int nEscravos, int vetEscravos[] )
	{   setIdCS( identCS );
		setTipo( tp );
		setNumMaxServidores( nMaxServ );
		setNumAtualServidores( 0 ); //Na inicializacao do CS nao ha adicao de servidores.
	    setNumServidoresLivres( getNumAtualServidores() );  //Inicialmente, nao ha servidor, entao nao ha servidor livre
	...
	//Adiciona CSs Heterogeneos (que tenham servidores de processamento e servidores de comunicacao
	//ou seja, CSs do tipo 1 (clusters) ).
	public CentrosDeServico( int identCS, int tp, int nMaxServ, int nMaxFilas, int servCom, int nEscravos, int vetEscravos[] )
	{   setIdCS( identCS );
		setTipo( tp );
		setNumMaxServidores( nMaxServ );
		setNumAtualServidores( 0 ); //Na inicializacao do CS nao ha adicao de servidores.
	...
	public CentrosDeServico( int identCS, int tp, int nMaxServ, int nMaxFilas, int servCom )
	{   setIdCS( identCS );
		setTipo( tp );
		setNumMaxServidores( nMaxServ );
		setNumAtualServidores( 0 ); //Na inicializacao do CS nao ha adicao de servidores.
	    System.out.printf("getNumAtualServidores() = %d", getNumAtualServidores() );
		setNumServidoresLivres( getNumAtualServidores() );  //Inicialmente, nao ha servidor, entao nao ha servidor livre
		setNumMaxFilas( nMaxFilas );
		setNumAtualFilas( 0 );
		if( tp == 1 )
	...
	{   tServicoProctoTotal = ( tServProctoTotal >= 0.0 ) ? tServProctoTotal : 0.0; 
	}
	...
	public void adicionaServidorProcto( int tpServ, boolean msOuEsc, Double tServProcto, MetricaCS atualCS )
	{   Servidores serv = new Servidores( getNumAtualServidores(), tpServ, msOuEsc, tServProcto );
		...
		servidores.add( serv ); // Adiciona no HashSet, ou seja, "add" eh um metodo da classe HashSet
		
		//MARCO
		atualCS.addMedidasServidor( getNumAtualServidores(), 0 );
		
		System.out.printf("|\tServidor ID: %2d adicionado ao Centro de Servico ID: %2d |\n",
		                   getNumAtualServidores(), getIdCS() );
		alteraNumAtualServidores( 1 ); //Soma 1 ao numero atual de servidores	
        alteraNumAtualServidoresLivres( 1 );// Soma 1 ao numero de servidores livres			
		
	}

	public void adicionaServidorCom( int tpServ, Double tServRede, double ltnc, double txOcup, MetricaCS atualCS )
	{   Servidores serv = new Servidores( getNumAtualServidores(), tpServ, tServRede, ltnc, txOcup );
	...
	public void adicionaServidoresInt( int tpServ, Double tServRede, double ltnc, double txOcup, MetricaCS atualCS )
	{   ehEscalonador = false;
...
//+--------------------------------+
	public void adicionaServidoresClr( int tpServ, Double tServProcto, Double tServRede, double ltnc, double txOcup, MetricaCS atualCS )
	{   Double tServInd;
		//MARCO
		metCSCDS = atualCS;
		...
		//Adiciona o mestre
		Servidores mestre = new Servidores( getNumAtualServidores(), tpServ, true, tServInd );
		//O "true" indica que os servidores sao escravos;
		servidores.add( mestre ); // Adiciona no HashSet, ou seja, "add" eh um metodo da classe HashSet
		...
		while( getNumAtualServidores() < getNumMaxServidores() )
		{   Servidores escravo = new Servidores( getNumAtualServidores(), tpServ, false, tServInd );
		    //O "false" indica que os servidores sao escravos;
			servidores.add( escravo ); // Adiciona no HashSet, ou seja, "add" eh um metodo da classe HashSet
		...
		//Apos adicionar todos os servidores de processamento, adiciono o servidor de comunicacao
		//que, pelo menos por enquanto, e unico (no cluster - barramento)
		
		//O id do servidor de comunicacao eh getNumAtualServidores()
		//pois os servidores de comunicacao sao numerados de 0 a getNumAtualServidores()-1
		//Ou seja, o proximo numero inteiro e getNumAtualServidores()
		System.out.printf("|\tServidor DE COMUNICACAO ID: %2d adicionado ao Centro de Servico ID: %2d |\n",
		                       getNumAtualServidores(), getIdCS() );
		Servidores serv = new Servidores( getNumAtualServidores(), 1, tServRede, ltnc, txOcup );  
		servidores.add( serv );
		
		//MARCO
		atualCS.addMedidasServidor( getNumAtualServidores(), 1 );
		
		//Nao precisa adicionar valor a  numAtualServidores, pois o servidor e de comunicacao e essa 
		//variavel controla o numero de servidores de processamento (ate pq, como dito logo acima, o 
		//servidor de comunicacao e unitario!
		
		//O "setamento" da variavel tServicoProctoTotal e so para "ter", pq ela soh e usada para "impressao"
		setTServicoProctoTotal( tServProcto );
		
	}	
//+--------------------------------Metodo que adiciona uma fila ao centro de servico-------------------------------+
	public void adicionaFila()
	{   Filas fila = new Filas( numAtualFilas );
		
		filas.add( fila );
		System.out.printf("|\tFila ID: %2d adicionada ao Centro de Servico ID: %2d     |\n", numAtualFilas, idCS);
	...
	{   Double retornoFuncao = 0.0;  //Eh o tempo que o sistema levou para adicionar uma tarefa na fila.
	    ...
	    { //  System.out.printf("temp.getIdFila() = %d\n", temp.getIdFila() );
		  //  System.out.printf("tarefa.getIdFila() = %d\n", tarefa.getIdFila() );
			if( temp.getIdFila() == tarefa.getIdFila() )   //Encontra-se a fila na qual se deseja adicionar a tarefa
		    { //  System.out.printf("if( temp.getIdFila() == tarefa.getIdFila() )" );
			    retornoFuncao = temp.adicionaTarefaFila( tarefa );   //Adiciona-se a tarefa
			}
		}
	...
	{   
		...
		{   if( temp.getIdFila() == 0 )  //A fila do Mestre de processamento e sempre a fila de id 0
		    {   return temp.peek();
            }					
		}
	    return null;
	...
	{   System.out.printf("getTipo() = %d \n", getTipo() );
		...
		if( getTipo() == 1 )   //Se for um cluster, pegar a tarefa na fila de comunicacao (id 1)
	    {  for( Filas temp : filas ) 
		   {   if( temp.getIdFila() == 1 )  //A fila de comunicacao de um cluster e sempre a fila de id 1
		       {   return temp.peek();  
               }					
		   }
		}
	    ...
	    {   
			...
			for( Filas temp : filas ) 
		    {   if( temp.getIdFila() == 0 )  //A fila de comunicacao de CSs tanto de conexao de rede (tipo 2)
		        {   System.out.printf("temp.getIdFila() == 0\n");
				    tarefaRetorno = temp.peek();
				    System.out.printf("id da tarefa peek %d\n", tarefaRetorno.getIdTarefa() );
				    return tarefaRetorno;      //quanto de Internet (tipo 3) e a fila principal, ou seja, id 0 
                }
		    }
	    }
		return null;
	...
	{   for( Filas temp : filas )
	    {   if( temp.getIdFila() == tarefa.getIdFila() )
		    {    System.out.printf("Tarefa %d retirada da fila %d do CS id %d \n", tarefa.getIdTarefa(), tarefa.getIdFila(), tarefa.getIdCSAtual() );
			     temp.poll();  // Nao eh uma chamada recursiva (ou seja, para o proprio metodo), mas 
	...
	{   Double retornoFuncao = 0.0;
		...
		{   for( Servidores servTemp : servidores )
		    {   if( servTemp.getIdServidor() == 0 )
		        {   retornoFuncao = servTemp.atribuiTarefaServComun( tarefa ) ;
            	    System.out.printf("A tarefa %d teria que ser alocada no servidor %d do CS %d\n", 
		               tarefa.getIdTarefa(), 0, tarefa.getIdCSAtual() );
		            System.out.printf("e foi alocada no CS %d\n", idCS );	
		               tarefa.setIdServidorAtual( 0 ); // Motivo do argumento ser 0: o id do servidor escravo
                                         // um cluster e sempre 0!	
					//MARCO
					atualCS.addNoMedidasServidor( 0, 1 , tarefa.getTamanhoComTarefa( ) / servTemp.getTServicoRede() + servTemp.getLatencia() );
		        }
			}	
		}
		else if( getTipo() == 1 )
		{   for( Servidores servTemp : servidores )
		    {   if( servTemp.getIdServidor() == getNumMaxServidores() )
		        {   retornoFuncao = servTemp.atribuiTarefaServComun( tarefa ) ;
            	    System.out.printf("A tarefa %d teria que ser alocada no servidor %d do CS %d\n", 
		               tarefa.getIdTarefa(), getNumMaxServidores(), tarefa.getIdCSAtual() );
		            System.out.printf("e foi alocada no CS %d\n", idCS );	
		               tarefa.setIdServidorAtual( getNumMaxServidores() ); // Motivo do argumento ser 0: o id do servidor escravo
                                         // um cluster e sempre 0!
					//MARCO
					atualCS.addNoMedidasServidor( servTemp.getIdServidor(), 1 , tarefa.getTamanhoComTarefa( ) / servTemp.getTServicoRede() + servTemp.getLatencia() );
		        }
			}	
		}
		return retornoFuncao;
	...
	{   Double retornoFuncao = 0.0;
		...
		{   if( servTemp.getIdServidor() == tarefa.getIdServidorAtual() )
		    {   retornoFuncao = tarefa.getTamanhoComTarefa( ) / servTemp.getTServicoRede() + servTemp.getLatencia();
            }
		}
	...
	{   Double retornoFuncao = 0.0;
		...
		for( Servidores servTemp : servidores ) 
		{   if( servTemp.getIdServidor() == tarefa.getIdServidorAtual() )
		    {   System.out.printf("O servidor %d do CS %d esta processando a tarefa %d\n", servTemp.getIdServidor(), getIdCS(), tarefa.getIdTarefa() );
				retornoFuncao = tarefa.getTamanhoProcTarefa( ) / servTemp.getTServicoProcto();
	...
	public Double atribuiTarefaServProc( NoFila tarefa, Double instanteOcupacaoServidor, MetricaCS atualCS )
	{   Double retornoFuncao = 0.0;
	    
		if( getTipo() == 0 )
		{   for( Servidores servTemp : servidores )
		    {   if( servTemp.getIdServidor() == 0 )
		        {   retornoFuncao = servTemp.atribuiTarefaServProc( tarefa, instanteOcupacaoServidor ) ;
            	    System.out.printf("A tarefa %d foi alocada no servidor %d do CS %d\n", 
		               tarefa.getIdTarefa(), tarefa.getIdServidorAtual(), tarefa.getIdCSAtual() );
					...
					//MARCO
					atualCS.addNoMedidasServidor( servTemp.getIdServidor(), 0 , tarefa.getTamanhoProcTarefa( ) / servTemp.getTServicoProcto() );
		        }
			}	
		}
		else if( getTipo() == 1 )
	...
	/* A versao abaixo do metodo VerificaMestreLivre( ) e chamada para um CS que contem um cluster (neste caso o mestre sempre
	...
	*/
	...
	public Double atribuiTarefaMestre( NoFila tarefa, int idCS, MetricaCS atualCS )
	{   Double retornoFuncao = 0.0;

		System.out.printf("A tarefa %d teria que ser alocada no MESTRE do CS %d\n", 
		   tarefa.getIdTarefa(), tarefa.getIdCSAtual() );
		System.out.printf("e foi alocada no CS %d\n", idCS );	
		   tarefa.setIdServidorAtual( 0 ); // Motivo do argumento ser 0: o id do servidor mestre de
                                         // um cluster e sempre 0!
	...
	{   Double retornoFuncao = 0.0;
	    for( Servidores servTemp : servidores )
		{   if( servTemp.getIdServidor() == tarefa.getIdServidorAtual() )
		    {   retornoFuncao = servTemp.getTempoPrevistoLiberacao( ) - tarefa.getTempoChegadaFila();	
			}
		}
		return retornoFuncao;
	}
	...
	{   int retornoFuncao = -1;
	    ...
	    {   if(  (1 <= indiceEscravoAtualRRCluster) && (indiceEscravoAtualRRCluster <= numMaxServidores-1) )
		    {   indiceEscravoAtualRRLocal = indiceEscravoAtualRRCluster;
                indiceEscravoAtualRRCluster++;
				System.out.printf("indiceEscravoAtualRRCluster (dentro)= %d\n", indiceEscravoAtualRRCluster);
			}

			escravoLivre = verificaSeServidorEstaLivre( indiceEscravoAtualRRLocal );
		}	
	...
	{   Double retornoFuncao = 0.0;
	    ...
	    for( Servidores servTemp : servidores )
		{   //A segunda condicao eh soh por garantia
	...
	/*O metodo abaixo e sobrescrito. Uma versao quando se da o id do Servidor diretamente e a outra 
	quando se da a tarefa pra ver se o servidor que ela vai (ou esta) esta livre.
	*/
	...
	{   boolean retornoFuncao = false;
	    for( Servidores servTemp : servidores )
		{   if( servTemp.getIdServidor() == tarefa.getIdServidorAtual() )
		    {   //System.out.printf("********servTemp.getIdServidor() = %d\n", servTemp.getIdServidor() );
			    retornoFuncao = servTemp.getServidorLivre() ;		
			}
		}
		return retornoFuncao;
	...
	{   int i, j;
	    ...
	    {   System.out.printf( "\tDados do servidor%d\n", servTemp.getIdServidor() );	
			...
			{   case 0: System.out.printf( "\tTipo: Processamento\n" );
				        ...
				        System.out.printf( "\n" );
            	        break;
				case 1: System.out.printf( "\tTipo: Comunicacao\n" );
