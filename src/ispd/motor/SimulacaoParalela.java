...
public class SimulacaoParalela extends Simulation {
     ...
     */
    public SimulacaoParalela(final ProgressoSimulacao window,
        ...
        this.recursos = new ArrayList<>();//Collections
        // .synchronizedList(new ArrayList<CentroServico>());
        ...
        for (final CentroServico rec : queueNetwork.getMestres()) {
            this.threadFilaEventos.put(rec,
                    new PriorityBlockingQueue<>());
            if (((CS_Mestre) rec).getEscalonador().getTempoAtualizar() != null) {
                this.threadTrabalhador.put(rec,
                        new ThreadTrabalhadorDinamico(rec,
                                this));
            } else {
                this.threadTrabalhador.put(rec, new ThreadTrabalhador(rec,
                        this));
            }
        }
    ...
    public boolean removeFutureEvent(final int eventType,
        ...
        for (FutureEvent ev : this.threadFilaEventos.get(eventServer)) {
    ...
    private class ThreadTrabalhador implements Runnable {
        ...
        ThreadTrabalhador(final CentroServico rec,
        ...
        double getRelogioLocal() {
            return this.relogioLocal;
        }

        void setRelogioLocal(final double relogio) {
        ...
        void setExecutando(final boolean executando) {
            this.executando = executando;
        }

        CentroServico getRecurso() {
            return this.recurso;
        }
        ...
        public void run() {
            ...
            synchronized (this) {
                while (!SimulacaoParalela.this.threadFilaEventos.get(this.recurso).isEmpty()) {
                    ...
                    //System.out.println(recurso.getId()+" vou executar:
                    // "+eventoAtual.toString()+" de "+threadFilaEventos.get
                    // (this.recurso).size());
                    if (Objects.requireNonNull(eventoAtual).getCreationTime() > this.relogioLocal) {
                        this.relogioLocal = eventoAtual.getCreationTime();
                    }
                    //if (this.relogioLocal > this.ultimaExec && recurso
                    // instanceof CS_Comunicacao) {
                    //    System.err.println(recurso.getId() + " Ocorreu erro
                    //    Grave! "
                    //            + this.relogioLocal + " > " + this
                    //            .ultimaExec);
                    ...
                    switch (eventoAtual.getType()) {
                        ...
                        case FutureEvent.ATENDIMENTO ->
                            ...
                            // eventoAtual.getServidor().getId() + " vou
                            // atender a tarefa " + eventoAtual
                            // .getTempoOcorrencia());
                                eventoAtual.getServidor().atendimento(this.simulacao,
                        ...
                        case FutureEvent.ESCALONAR ->
                                eventoAtual.getServidor().requisicao(this.simulacao,
                                        null, FutureEvent.ESCALONAR);
                        default ->
                                eventoAtual.getServidor().requisicao(this.simulacao,
                ...
                //    System.out.println(recurso.getId() + " chamada sem
                //    evento!");
                //}
                this.executando = false;
            }
    ...
    private class ThreadTrabalhadorDinamico extends ThreadTrabalhador {
        ...
        private ThreadTrabalhadorDinamico(final CentroServico rec,
                                          final Simulation sim) {
            super(rec, sim);
        ...
        public void run() {
            ...
            synchronized (this) {
                while (!SimulacaoParalela.this.threadFilaEventos.get(this.getRecurso()).isEmpty()) {
                    if ((Double) this.item[2] < Objects.requireNonNull(SimulacaoParalela.this.threadFilaEventos.get(this.getRecurso()).peek()).getCreationTime()) {
                    ...
                    if (Objects.requireNonNull(eventoAtual).getCreationTime() > this.getRelogioLocal()) {
