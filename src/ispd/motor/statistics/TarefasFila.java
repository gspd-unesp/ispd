...
public class TarefasFila {
    ...
    public void imprimeTarefas() {
        System.out.printf("\nFila DO TIPO: %d\n", getTipoMetrica());
        for (NoTarefasFila temp : fila) {
            System.out.printf("Tarefa ID: %d --> Tamanho %g --> " +
							  "TempoMedioFila --> %f TempoMedioServidor --> %f" +
							  " TempoMedioSistema --> %f\n",
					temp.getIdTarefa(), temp.getTamanhoTarefa(),
					getTempoMedioFila(), getTempoMedioServidor(),
					getTempoMedioSistema());
        }
    }
    ...
    public void setTempoMedioFila(int id) {
        ...
        for (NoTarefasFila no : fila) {
            if (no.getIdTarefa() == id) {
                tempoMedioFila =
						tempoMedioFila + (no.getTempoChegadaServidor() - no.getTempoChegadaFila());
                break;
            }
        }
    ...
    public void setTempoMedioServidor(int id) {
        ...
        for (NoTarefasFila no : fila) {
            if (no.getIdTarefa() == id) {
                tempoMedioServidor =
						tempoMedioServidor + (no.getTempoSaidaSistema() - no.getTempoChegadaServidor());
                break;
            }
        }
    ...
    public void setTempoMedioSistema(int id) {
        ...
        for (NoTarefasFila no : fila) {
            if (no.getIdTarefa() == id) {
                tempoMedioSistema =
						tempoMedioSistema + (no.getTempoSaidaSistema() - no.getTempoChegadaFila());
                break;
            }
        }
        ...
        for (NoTarefasFila no : fila) {
            if (no.getIdTarefa() == idTarefa) {
                if (no.getTempoSaidaSistema() == 0.0) {
                    ...
                    //no.setTempoSaidaSistema( tempo );
                    System.out.printf("ENTRA NO ELSE DA SAIDA DO SISTEMA A " +
									  "TAREFA %d DO TIPO DE FILA %d !\n",
    ...
    public void setNumeroTarefasConcluidas(int tarefasConcluidas) {
        numeroTarefasConcluidas = (tarefasConcluidas >= 0) ?
				tarefasConcluidas : 0;
    }
